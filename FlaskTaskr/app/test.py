...
class AllTests(unittest.TestCase):
	...
	def test_users_can_login(self):
		self.register('Michael', 'michael@gmail.com', 
					'python', 'python')
		response = self.login('Michael', 'python')
		self.assertIn(b'You are logged in. Go crazy', response.data)
	...
	def test_logged_in_users_can_logout(self):
		...
		self.asserIn(b'Goodbye', response.data)
	...
	def test_not_logged_in_users_cannot_access_tasks_page(self):
		...
		self.assertIn(b'You need login first.', response.data)
	...
	def test_users_cannot_complete_tasks_that_are_not_created_by_them(self):
		...
		self.login('Fletcher', 'python101')
		self.get('tasks/', follow_redirects=True)
		response = self.app.get('complete/1/', follow_redirects=True)
